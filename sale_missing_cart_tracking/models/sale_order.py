...
class SaleOrder(models.Model):
    ...
    def _get_missing_product_domain(self):
        ...
        domain = [
            ...
            ("product_id", "not in", (self.order_line.mapped("product_id") + missing_product_exception).ids),
            ("product_id.sale_missing_tracking", "=", True),
            ("state", "in", ["sale", "done"]),
            ("order_id.date_order", ">=", now - relativedelta(
                days=self.company_id.sale_missing_days_from)),
            ("order_id.date_order", "<=",
